const {
  ...
  describe('FeiStakingRewards', function () {
    ...
    beforeEach(async function () {
      ...
      await this.core.allocateMono(this.staking.address, this.rewardAmount, {from: governorAddress});
    });
  
    describe('Init', function() {
      it('rewardsToken', async function() {
        expect(await this.staking.rewardsToken()).to.be.equal(this.mono.address);
      });
    ...
    describe('recoverERC20', function() {
        describe('non-owner', function() {
            it('reverts', async function() {
                await expectRevert(this.staking.recoverERC20(this.mono.address, userAddress, this.rewardAmount, {from: userAddress}), "Caller is not RewardsDistribution contract");
            });
        });
        describe('invalid token', function() {
            it('rewards reverts', async function() {
                await expectRevert(this.staking.recoverERC20(this.mono.address, governorAddress, this.rewardAmount, {from: governorAddress}), "Cannot withdraw the rewards token");
            });
    ...
    describe('exit', function() {
        ...
        describe('alone from start', async function() {
            ...
            it('earns rewards', async function() {
                ...
                expect(await this.mono.balanceOf(userAddress)).to.be.bignumber.equal(this.rewardAmount);
            });
        ...
        describe('alone from halfway', async function() {
            ...
            it('earns rewards', async function() {
                expect(await this.staking.rewards(userAddress)).to.be.bignumber.equal(new BN('0'));
                expect(await this.staking.earned(userAddress)).to.be.bignumber.equal(new BN('0'));
                expect(await this.mono.balanceOf(userAddress)).to.be.bignumber.equal(this.rewardAmount.div(new BN('2')));
            });
        ...
        describe('second from halfway 50/50', async function() {
            ...
            it('earns rewards', async function() {
                expect(await this.staking.rewards(userAddress)).to.be.bignumber.equal(new BN('0'));
                expect(await this.staking.earned(userAddress)).to.be.bignumber.equal(new BN('0'));
                expect(await this.mono.balanceOf(userAddress)).to.be.bignumber.equal(this.rewardAmount.div(new BN('4')));
            });
