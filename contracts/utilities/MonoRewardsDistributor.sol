...
/// @title Distributor for MONO rewards to the staking contract
/// @author Mono Protocol
/// @notice distributes MONO over time at a linearly decreasing rate
contract MonoRewardsDistributor is IRewardsDistributor, CoreRef, Timed {
    ...
    constructor(
        ...
        Timed(_duration)
    {
        require(_duration >= _frequency, "MonoRewardsDistributor: frequency exceeds duration");
    ...
    /// @notice sends the unlocked amount of MONO to the stakingRewards contract
    /// @return amount of MONO sent
    function drip() public override postGenesis whenNotPaused nonContract returns(uint256) {
        require(isDripAvailable(), "MonoRewardsDistributor: Not passed drip frequency");
        // solhint-disable-next-line not-rely-on-time
        lastDistributionTime = block.timestamp;

        uint amount = releasedReward();
        require(amount != 0, "MonoRewardsDistributor: no rewards");
        distributedRewards = distributedRewards.add(amount);

        mono().transfer(address(stakingContract), amount);
    ...
    function governorWithdrawMono(uint256 amount) external override onlyGovernor {
        mono().transfer(address(core()), amount);
        emit MonoWithdraw(amount);
    }
    ...
    /// @notice sets the staking contract to send MONO rewards to
